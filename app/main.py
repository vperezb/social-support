...
def offer():
    ...
    if z:
        lat, lng, zoom, radius = z.split(',')

        return render_template(
            'offer.html', 
            context={'key':'AIzaSyAnmtePzc5y5TgEIj9ewyBDNyRvcNFzuNw'}, 
            zone={'lat':lat, 'lng':lng, 'zoom':zoom, 'radius':radius}, 
            needs=[{'lat': 2,'lng':40, 'id':'aaa'},{'lat': 39,'lng':1.2, 'id':'bbb'}])
    else:
        return render_template(
            'offer.html', 
            context={'key':'AIzaSyAnmtePzc5y5TgEIj9ewyBDNyRvcNFzuNw'}, 
            zone={'lat':40, 'lng':2, 'zoom':8, 'radius':20000}, 
            needs=[{'lat': 2,'lng':40, 'id':'aaa'},{'lat': 39,'lng':1.2, 'id':'bbb'}])
@app.route('/need')
def need():
